...
def sgc_objective(space,try_time=10, long_epoch=False):
    features, precompute_time = dgc_precompute_lhgaussian_taylor(raw_features, adj, 100, space['T1'],space['T2'],-1,1)#-1+math.sqrt((math.sqrt(2))/space['T']),1-math.sqrt(math.log(math.sqrt(2))/space['T']))## Fix Degree to be 5
    res = []
    running_time = []
